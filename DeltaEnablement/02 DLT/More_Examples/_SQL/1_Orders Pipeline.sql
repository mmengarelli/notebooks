...
-- MAGIC 
-- MAGIC Delta Live Tables adapts standard SQL queries to combine DDL (data definition language) and DML (data manipulation language) into a unfied declarative syntax.
-- MAGIC 
...
-- MAGIC 
-- MAGIC The two queries we've reviewed so far have both created streaming tables. Below, we see a simple query that returns a materialized view of some aggregated data.
-- MAGIC 
...
-- MAGIC 
-- MAGIC Note the only syntactic differences between streaming tables and materialized views are the lack of the `STREAMING` keyword in the create clause and not wrapping the source table in the `STREAM()` method.
-- MAGIC 
