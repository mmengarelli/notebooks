...
-- MAGIC * `APPLY CHANGES INTO` is used to automatically process CDC data into the silver layer as a Type 1 [slowly changing dimension (SCD) table](https://en.wikipedia.org/wiki/Slowly_changing_dimension)
...
-- MAGIC 
-- MAGIC Views in DLT differ from persisted tables, and can optionally be defined as `STREAMING`, as seen here.
-- MAGIC 
...
CREATE STREAMING LIVE VIEW customers_bronze_clean_v
...
-- MAGIC * Specifies the view `customers_bronze_clean_v` as the streaming source
...
APPLY CHANGES INTO LIVE.customers_silver
  FROM STREAM(LIVE.customers_bronze_clean_v)
  KEYS (customer_id)
