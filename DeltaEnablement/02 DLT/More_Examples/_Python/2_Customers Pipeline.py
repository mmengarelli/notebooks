...
# MAGIC * A view is defined to enforce constraints before passing records to the silver layer
# MAGIC * `dlt.apply_changes()` is used to automatically process CDC data into the silver layer as a Type 1 [slowly changing dimension (SCD) table](https://en.wikipedia.org/wiki/Slowly_changing_dimension)
...
# MAGIC 
# MAGIC In the example below, we use this option to fulfill a table naming convention of `<dataset-name>_<data-quality>` and a function naming convention that describes what the function is doing.
...
# MAGIC 
# MAGIC Views in DLT differ from persisted tables, and can also inherit streaming execution from the function they decorate, as seen here.
# MAGIC 
...
# MAGIC * Specifies the view `customers_bronze_clean_v` as the source (**NOTE**: source must be append-only)
...
dlt.apply_changes(
    ...
    source = "customers_bronze_clean_v",
    keys = ["customer_id"],
...
# MAGIC 
# MAGIC While the target of our operation in the previous cell was defined as a streaming table, downstream operations should query this table as if it were a materialized view.
# MAGIC 
...
# MAGIC 
# MAGIC Below, we define a simple aggregate query to create a materialized view from the data in the `customers_silver` table.
...
# MAGIC 
# MAGIC In the query below, we create a new materialized view by joining the silver tables from our `orders` and `customers` datasets. Because this is a materialized view, we will always capture the current valid `email` for each customer, and will automatically drop records for customers after they've been deleted from the `customers_silver` table.
...
def subscribed_order_emails():
    return (
        dlt.read_stream("orders_silver").alias("a")
            .join(
