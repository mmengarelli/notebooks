...
# MAGIC * Describing differences in syntax and execution of streaming tables and materialized views
...
# MAGIC 
# MAGIC Here, we should importing `pyspark.sql.functions` as `F`. Some developers import `*`, while others will only import the functions they need in the present notebook. These lessons will use `F` throughout so that students clearly know which methods are imported from this library.
...
# MAGIC * **Materialized views** will return the current results of any query with each refresh
# MAGIC * **Streaming tables** are designed for incremental, near-real time data processing
# MAGIC 
# MAGIC Note that both of these objects are persisted as tables stored with the Delta Lake protocol (providing ACID transactions, versioning, and many other benefits). We'll talk more about the differences between streaming tables and materialized views later in the notebook.
# MAGIC 
...
# MAGIC 
# MAGIC There are [a number of options](https://docs.databricks.com/data-engineering/delta-live-tables/delta-live-tables-python-ref.html#create-table) that can be specified during table creation. Here, we use two of these to annotate our dataset.
# MAGIC 
# MAGIC ##### `comment`
# MAGIC 
...
# MAGIC 
# MAGIC The Python version of DLT uses decorator functions to set [data quality constraints](https://docs.databricks.com/data-engineering/delta-live-tables/delta-live-tables-expectations.html#delta-live-tables-data-quality-constraints). We'll see a number of these throughout the course.
# MAGIC 
...
# MAGIC %md
# MAGIC ## Materialized Views vs. Streaming Tables
# MAGIC 
# MAGIC The two functions we've reviewed so far have both created streaming tables. Below, we see a simple function that returns a materialized view of some aggregated data.
# MAGIC 
# MAGIC Spark has historically differentiated between batch queries and streaming queries. While materialized views borrow the syntax of batch queries, the big difference here is that DLT seeks to optimize performance by materializing these results incrementally when this is more efficient.
# MAGIC 
...
# MAGIC * Return results as if they are recomputing the entire source dataset, but may optimize with incremental processing under the hood.
